        ...
        <div class="col-md-4 p-3">
            <div class="card product-card h-100">
                ...
                <div class="card-body text-center">
                    ...
                    <button class="btn btn-primary add-cart-btn" data-index="${index}">
                        <i class="fa fa-cart-plus"></i> Add to Cart
                    </button>

                    <button class="btn btn-outline-danger favorite-btn" data-index="${index}">
...
for (let i = 0; i < favButtons.length; i++) {
    favButtons[i].onclick = function () {
        ...
        for (let j = 0; j < favs.length; j++) {
            if (favs[j].name === product.name && favs[j].image === product.image) {
                exists = true;
                break;
            }
        }
...
for (let i = 0; i < cartButtons.length; i++) {
    cartButtons[i].onclick = function () {
        ...
        for (let j = 0; j < cart.length; j++) {
            if (cart[j].name === product.name && cart[j].image === product.image) {
                cart[j].quantity += 1;
                exists = true;
                break;
            }
